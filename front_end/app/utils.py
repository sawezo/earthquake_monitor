...
def frame_data(messages:List[dict]) -> pd.DataFrame:
    quakes = [flatten_json(q) for message in messages for q in message["features"]]
    return pd.DataFrame.from_records(quakes)